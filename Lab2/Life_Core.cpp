...
namespace life {
    ...
    Life::Life(const std::vector<bool> &univercity_, size_t weight, size_t height) : weight_(weight),
                                                                                     height_(height),
                                                                                     univercity(univercity_) {
        this->neighbours.resize(this->weight_ * this->height_);
        std::fill(this->neighbours.begin(), this->neighbours.end(), 0);
    }

    Life::Life(const std::vector<bool> &univercity_, std::array<bool, 9> when_survival_,
               std::array<bool, 9> when_birth_, size_t weight, size_t height) : weight_(weight), height_(height),
                                                                                when_birth(when_birth_),
                                                                                when_survival(when_survival_),
                                                                                univercity(univercity_) {
        this->neighbours.resize(this->weight_ * this->height_);
        std::fill(this->neighbours.begin(), this->neighbours.end(), 0);
    }
